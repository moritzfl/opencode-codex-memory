Consolidate the supplied session recaps into `memory_summary.md` so another
...
decisions, or provenance; redact secrets and access-bearing URL values as
[REDACTED:secret].
...
- rollout_summaries/<exact supplied filename> — <one semantic sentence explaining what it contains and when it matters>; session_id=<exact complete source session identifier>
...
description and either the exact filename or complete session identifier.
...
summary if no supported content remains. You have read, edit, write, glob, and
grep only — you cannot delete files or run a shell.
