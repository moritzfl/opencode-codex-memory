...
1. Read `{{ diff_path }}` to see what's new or changed. It contains a `## Status` listing (A/M/D per file) and a `## Diff` section with the unified diff since the last consolidation (truncated if very large). Do not edit this file.
