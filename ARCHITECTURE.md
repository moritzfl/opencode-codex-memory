...
| Rate-limit awareness | Provider rate-limit info (`min_rate_limit_remaining_percent`), fail open | Phase-1 30s anti-stampede plus an observed-quota circuit breaker (1h, both phases); quota failures do not burn stage-1 retries | See `src/ratelimit.ts`; wire live provider quota when opencode exposes it |
